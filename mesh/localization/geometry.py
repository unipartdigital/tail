...
def drawC(cA, ax):
    ...
    for q, w in enumerate(cA):
        ...
        if t2 > ymax:
            ymax = t2
        circ = pl.Circle((w.c.x, w.c.y), radius=w.r, fc='none', lw=2,
                         ec=pickColor(q))
        ax.add_artist(circ)
...
class vec:
    ...
    def __str__(self):
        return 'vec(' + str(self.dx) + ',' + str(self.dy) + ',' + str(
            self.dz) + ')'
...
class circle:
    ...
    def to_poly(self, step):
        ...
        while alpha < (2 * math.pi):
            x = self.c + point(self.r * math.cos(alpha),
                               self.r * math.sin(alpha))
            pll.append(x)
            alpha = alpha + step
        pll[-1] = pll[0]
        return pll
...
class Triangle:
    ...
    def __str__(self):
        return 'Triangle{' + self.a.__str__().strip(
            'p') + ',' + self.b.__str__().strip(
...
class Sphere:
    ...
    def gcd(self, lon1, lat1, lon2, lat2):
        ...
        lon1, lat1, lon2, lat2 = list(
            map(math.radians, [lon1, lat1, lon2, lat2]))
...
class Ray:
    ...
    def to_line(self):
        if (abs(self.a - math.pi / 2) < res) or (
            abs(self.a + math.pi / 2) < res):
            m = float('inf')
            b = self.c.x
            return line(m, b)
        m = math.tan(self.a)
