        ...
        if bc == '10' or bc == '01':
            ...
            for k, xx in enumerate(lon[1:]):
                if fg:
                    ...
                    if abs(xx - pivot) > min(90, 10 * sdr):  # Passing (+/-)180
                        # meridian for the second time
                        j = k
                        break
                    pivot = xx
                    # Note that a small circle can not pass meridian more
                    # than twice
            if ii == -1 and j == -1:
                ...
                raise gx.geoError(
                        'Algorithmic bug in polygonizing for earth model ')
            else:
                lon1 = lon[ii + 1:j + 1]
                lat1 = lat[ii + 1:j + 1]
                ...
                if lon1[0] > lon2[0]:
                    P1 = Polygon([(180, lat1[0])] + list(zip(lon1, lat1)) + [
                        (180, lat1[-1])])
                    P2 = Polygon([(-180, lat2[0])] + list(zip(lon2, lat2)) + [
                        (-180, lat2[-1])])
                else:
                    P1 = Polygon([(-180, lat1[0])] + list(zip(lon1, lat1)) + [
                        (-180, lat1[-1])])
