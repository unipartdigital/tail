...
def ExportData(frame):
    if cfg.data_file is None:
        return
    bid = frame.tail_beacon - cfg.blink_skip
    if bid < 0:
        return
    if bid > cfg.blink_count:
        return
    ...
    line += ',{}'.format(eui)			# 1
    line += ',0x{:x}'.format(int(tstamp.sw))	# 2
    line += ',0x{:x}'.format(int(tstamp.hires))	# 3
    ...
    line += ',{:.3f}'.format(cfg.distance) 	# 23
    line += '\n'
    cfg.data_file.write(line)
    cfg.data_file.flush()


def SendBeacon(rsock,bid,when):
    frame = TailFrame()
...
class rxThread(threading.Thread):
    ...
    def recv_time(self):
        ...
        if frame.tail_protocol == 1 and frame.tail_frmtype == 1:
            frame.timestamp.tsinfo.cir_pwr = int(cfg.dw1000_power,0)
            ExportData(frame)
            if cfg.print_frames:
                print(frame)
...
def SocketLoop():
    ...
    try:
        pipe = tpipe.accept()
        pr.debug('New connection accepted')
...
def main():
    ...
    parser.add_argument('-d', '--distance', type=float, default=cfg.distance)
    ...
    if args.csv is not None:
        if args.csv.endswith('.gz'):
            cfg.data_file = gzip.open(args.csv,'wt+')
        else:
